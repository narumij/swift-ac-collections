...
void *cpp_map_create(const intptr_t *start, size_t count) {
...
void cpp_map_destroy(void *ptr) { delete static_cast<custom_map *>(ptr); }

void cpp_map_from_int_range(intptr_t count) {
  auto map = custom_map();
...
void cpp_map_insert_integers(const intptr_t *start, size_t count) {
  auto map = custom_map();
...
__attribute__((noinline)) auto find(custom_map *map, intptr_t value) {
  return map->find(value);
}

//void cpp_map_lookups(void *ptr, const intptr_t *start, size_t count) {
//  auto map = static_cast<custom_map *>(ptr);
...
void cpp_map_subscript(void *ptr, const intptr_t *start, size_t count) {
  auto map = static_cast<custom_map *>(ptr);
  for (auto it = start; it < start + count; ++it) {
    black_hole((*map)[*it]);
  }
...
void cpp_map_removals(void *ptr, const intptr_t *start, size_t count) {
  auto map = static_cast<custom_map *>(ptr);
  for (auto it = start; it < start + count; ++it) {
