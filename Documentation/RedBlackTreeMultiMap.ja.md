...
lower-bound と upper-bound を組み合わせることで、
そのキーに対応する要素範囲を効率よく求められます。
...
同じキーに複数の値を関連付けながら、
キーのソート順を維持し、キー範囲に基づく検索を行いたい場合に適しています。

単にキーごとに複数の値をまとめて保持するだけでよく、
個々のキー・値の組を独立したソート済み要素として扱う必要がない場合は、
