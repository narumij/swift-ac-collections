...
赤黒木モジュールの各データ構造はノードを配列に格納しています。
この内部配列のインデックス同士を比較した場合、順序と結果が一致しません。

Swift標準のプロトコルではインデックスと順序の一致を要求するため、抽象化されたインデックスを利用しています。
これが`Index`です。実際的にはノードへのポインタやC++のイテレータのような働きをします。
大半のAPIは`Index`を用いています。

他に`RawIndex`があり、これは比較はできませんが軽量で、要素アクセスや要素削除に利用できます。
