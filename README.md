...
この問題を迂回しない楽な使い方です。
一時できるようになり、紆余曲折のすえに、安定してできるようになりました。
IndexをStridableにした際に`Range<index>`に生えるシーケンスが要素をとる毎に範囲チェックをして
それがO(log *n*)あり、とても遅かったのですが、自前でイテレータやシーケンスを用意して上書きしたため、
ようやく普通に動くようになりました。

普通に動きますが落とし穴にハマる場合もあるので、油断しないでください。
