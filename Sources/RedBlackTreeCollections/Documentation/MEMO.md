...
- 範囲制限にむいてない。O(log n)のチェックコストが毎度発生する
- 玄人向きと割り切った方がよさそう
...
生メモリを触るため未定義動作リスクがあるが、鉄壁で防ぐ
関係ないところは触れないよう、壊さないようにすること。
が、O(log N)なチェックはできるだけ避けたい。

### inlinable

ジェネリクスやプロトコルはwitness tableの低速を避けたいので、常に、`@inlinable`。
中途半端にalways付与するとオプティマイザの邪魔になり、まぐれでしか速くならないので、
サブスクリプト、サブスクリプトヘルパー、アクセサコルーチン、アクセスハンドラ以外は`@inline(__always)`を極力付与しない
（そもそも構造が複雑なので標準コンテナやswift-collectionsのまねをしても同じ結果にならない）
...
- 配列方式はポインタと比較した場合、オフセット計算が発生するため定数倍でどうしても差が残ってしまう
- 参照型を利用すると参照カウントやメモリ管理のオーバーヘッドがつきまとう
- Swiftでは絶対に埋まらない差がどうしても残る
- 本来の性能にさらに近づけるためには、Cによる実装とCinteropでの利用が必要そう
