...
// - 現段階では、わずかな性能差のために格納値や参照経路を増やすのはworkaround過剰とも
//   判断した。中心となる設計は、genericなinitでpayload依存の情報を型消去し、その後は
//   型消去済みの情報だけで安全かつ低コストに処理することである。最適化手段がこの設計を
//   見えにくくしたり、compilerの細かな挙動への対処を際限なく増やしたりしないようにする。
