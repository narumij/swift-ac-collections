...
@frozen
@usableFromInline
struct _KeyValue_FindEqual<_Key, _MappedValue> where _Key: Comparable {
  ...
  public typealias _PayloadValue = RedBlackTreePair<_Key, _MappedValue>

  @usableFromInline let header: UnsafeMutablePointer<UnsafeTreeV2BufferHeader>
}

extension _KeyValue_FindEqual {
...
extension _KeyValue_FindEqual {
    ...
    __find_equal(_ __v: _Key) -> (__parent: _NodePtr, __child: _NodeRef)
  {
    ...
    while true {
      ...
      let __comp_res = __comp(__v, __nd.__value_(as: _PayloadValue.self).pointee.key)
