...
extension ___RedBlackTreeMerge {
  ...
  where Source: MergeSourceProtocol, Source._Key == _Key, Source.Element == Element {
    var __i = __source.__begin_node; while __i != __source.__end_node() {
  ...
  @inlinable
  mutating func ___tree_merge_unique<Source,Key,Value>(_ __source: Source, uniquingKeysWith combine: (Value, Value) throws -> Value) rethrows
  ...
  @inlinable
  mutating func ___merge_unique<S,Key,Value>(_ __source: S, uniquingKeysWith combine: (Value, Value) throws -> Value) rethrows
  where Element == _KeyValueTuple_<Key,Value>,
