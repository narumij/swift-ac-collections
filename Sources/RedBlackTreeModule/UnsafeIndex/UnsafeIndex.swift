...
@frozen
public struct UnsafeIndex<Base> where Base: ___TreeBase & ___TreeIndex {
...
#if DEBUG
  extension UnsafeIndex {
    fileprivate init(_unsafe_tree: UnsafeTree<Base>, rawValue: _NodePtr, node_id: Int) {
...
#if swift(>=5.5)
  extension UnsafeIndex: @unchecked Sendable
...
extension UnsafeIndex {
  @inlinable
  @inline(__always)
  internal var ___indices: UnsafeIndices<Base> {
...
@inlinable
@inline(__always)
public func + <Base>(lhs: UnsafeIndex<Base>, rhs: Int) -> UnsafeIndex<Base> {
  lhs.advanced(by: rhs)
}
...
@inlinable
@inline(__always)
public func - <Base>(lhs: UnsafeIndex<Base>, rhs: UnsafeIndex<Base>) -> Int {
  rhs.distance(to: lhs)
}
