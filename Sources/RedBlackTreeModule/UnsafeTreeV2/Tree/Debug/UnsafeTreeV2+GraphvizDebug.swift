...
import Foundation
  ...
  extension UnsafeTreeV2 {
    ...
    func buildGraphviz() -> Graphviz.Digraph {
      ...
      let reds = (0..<initializedCount).map{ self[_raw: $0] }.filter(isRed).map(nodeV)
      let blacks = (0..<initializedCount).map{ self[_raw: $0] }.filter(isBlack).map(nodeV)
      let lefts: [(_NodePtr, _NodePtr)] = (0..<initializedCount).map{ self[_raw: $0] }.filter(hasLeft).map(leftPair)
      let rights: [(_NodePtr, _NodePtr)] = (0..<initializedCount).map{ self[_raw: $0] }.filter(hasRight).map(rightPair)
