...
struct FreshStorage {
  @inlinable
  @inline(__always)
  internal init(capacity: Int, buffer: UnsafeMutableRawPointer, pointer: UnsafeMutablePointer<FreshStorage>? = nil) {
...
struct FreshPool<_Value> {
  @usableFromInline
  internal init(storage: UnsafeMutablePointer<FreshStorage>? = nil, array: UnsafeMutablePointer<UnsafeMutablePointer<UnsafeNode>>? = nil) {
...
//                           pointer: storage))
    let newStorage = createBucket(capacity: size).0
    storage = newStorage
    return UnsafePair<_Value>.pointer(from: newStorage.pointee.buffer)
  }
  ...
  @inlinable
  @inline(__always)
  func createBucket(capacity: Int) -> (UnsafeMutablePointer<FreshStorage>, capacity: Int) {
