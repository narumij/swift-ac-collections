...
@usableFromInline
protocol SafeEraseProtocol: EraseInterface, EndNodeInterface {}

extension SafeEraseProtocol {
  
  /// 最悪でもendで止まる
  @inlinable
  @inline(__always)
  internal func
  ___safe_erase(_ __f: _NodePtr, _ __l: _NodePtr) -> _NodePtr
  {
    var __f = __f
    while __f != __l, __f != __end_node {
      __f = erase(__f)
    }
    return __l
  }
...
@usableFromInline
protocol SafeEraseMultiProtocol: _KeyType, EqualInterface, EraseInterface, EndNodeInterface {}

extension SafeEraseMultiProtocol {

  /// 最悪でもendで止まる
  @inlinable
  @inline(__always)
  internal func ___safe_erase_multi(_ __k: _Key) -> Int {
    var __p = __equal_range_multi(__k)
    var __r = 0
    while __p.0 != __p.1, __p.0 != __end_node {
      defer { __r += 1 }
      __p.0 = erase(__p.0)
    }
    return __r
