...
final class DictionaryPointerTests: XCTestCase {
  ...
  func testPointerOffset0() throws {
    XCTAssertEqual((members.startIndex).pointee?.key, 0)
    XCTAssertEqual(members.startIndex.offset(by: 1)?.pointee?.key, 1)
    XCTAssertEqual(members.startIndex.offset(by: 2)?.pointee?.key, 2)
    XCTAssertEqual(members.startIndex.offset(by: 3)?.pointee?.key, 3)
    XCTAssertEqual(members.startIndex.offset(by: 4)?.pointee?.key, 4)
  ...
  func testPointerOffset2() throws {
    XCTAssertNil((members.endIndex).pointee)
    XCTAssertEqual(members.endIndex.offset(by: -1)?.pointee?.key, 4)
    XCTAssertEqual(members.endIndex.offset(by: -2)?.pointee?.key, 3)
    XCTAssertEqual(members.endIndex.offset(by: -3)?.pointee?.key, 2)
    XCTAssertEqual(members.endIndex.offset(by: -4)?.pointee?.key, 1)
    XCTAssertEqual(members.endIndex.offset(by: -5)?.pointee?.key, 0)
