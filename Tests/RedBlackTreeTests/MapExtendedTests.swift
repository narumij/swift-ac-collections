...
final class MapExtendedTests: XCTestCase {
  
    // MARK: ── ヘルパ ────────────────────────────────────────────────

    private func assertEqual<K: Comparable, V: Equatable>(
        _ rb: RedBlackTreeMap<K, V>,
        _ swift: [K: V],
    ...
    func testLiteralInitializers() {
        let fromDictLit: RedBlackTreeMap = ["a": 1, "b": 2]
        XCTAssertEqual(fromDictLit["b"], 2)

        let fromArrLit: RedBlackTreeMap = [("x", 10), ("y", 20)]
    ...
    func testMergeAndMergingAPIs() {
        ...
        let other: RedBlackTreeMap = [("k", 9), ("n", 4)]
    ...
    func testPopFirstReturnsMinKey() {
        var d: RedBlackTreeMap = [("b", 2), ("a", 1), ("c", 3)]
    ...
    func testMapAndCompactMapValues() {
        let src: RedBlackTreeMap = ["one": 1, "two": 2]
    ...
    func testFilterReturnsDictionary() {
        let d: RedBlackTreeMap = [1:"x", 2:"y", 3:"z"]
    ...
    func testBasicCrudAndIndexInvalidation() {
        var dict: RedBlackTreeMap = ["p": 9, "q": 8]
    ...
    func testFuzzEquivalence() {
        var rng = SplitMix64(seed: 0x1337C0DE)
        for _ in 0..<120 {
            var rb = RedBlackTreeMap<Int, Int>()
            var std = [Int: Int]()
