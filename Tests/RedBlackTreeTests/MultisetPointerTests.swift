...
final class MultisetPointerTests: XCTestCase {
  ...
  func testPointerOffset0() throws {
    XCTAssertEqual((members.startIndex).pointee, 0)
    XCTAssertEqual(members.startIndex.offset(by: 1)?.pointee, 0)
    XCTAssertEqual(members.startIndex.offset(by: 2)?.pointee, 1)
    XCTAssertEqual(members.startIndex.offset(by: 3)?.pointee, 2)
    XCTAssertEqual(members.startIndex.offset(by: 4)?.pointee, 2)
  ...
  func testPointerOffset2() throws {
    XCTAssertNil((members.endIndex).pointee)
    XCTAssertEqual(members.endIndex.offset(by: -1)?.pointee, 2)
    XCTAssertEqual(members.endIndex.offset(by: -2)?.pointee, 2)
    XCTAssertEqual(members.endIndex.offset(by: -3)?.pointee, 1)
    XCTAssertEqual(members.endIndex.offset(by: -4)?.pointee, 0)
