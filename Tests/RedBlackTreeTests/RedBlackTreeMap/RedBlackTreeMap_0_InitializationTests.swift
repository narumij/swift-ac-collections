...
final class RedBlackTreeMapInitializationTests: XCTestCase {
  ...
  func testInitEmptyDictionary() {
    let dict = RedBlackTreeMap<String, Int>()
  ...
  func testInitWithUniqueKeysFromArray() {
    let dict = RedBlackTreeMap<String, Int>(uniqueKeysWithValues: elements)
  ...
  func testInitWithUniqueKeysFromAnySequence() {
    let dict = RedBlackTreeMap<String, Int>(uniqueKeysWithValues: AnySequence(elements))
  ...
  func testInitWithDuplicateKeysMergingValues() throws {
    let dict = RedBlackTreeMap<String, Int>(duplicate, uniquingKeysWith: +)
  ...
  func testInitWithDuplicateKeysMergingValuesAnySequence() throws {
    let dict = RedBlackTreeMap<String, Int>(AnySequence(duplicate), uniquingKeysWith: +)
  ...
  func testInitGroupingValuesByFirstCharacter() throws {
    let dict = RedBlackTreeMap<String, [String]>(grouping: items, by: { String($0.first!) })
  ...
  func testInitGroupingValuesByFirstCharacterAnySequence() throws {
    let dict = RedBlackTreeMap<String, [String]>(grouping: AnySequence(items), by: { String($0.first!) })
  ...
  func testInitWithDuplicateKeysShouldCauseFatalError() {
    ...
    // _ = RedBlackTreeMap(uniqueKeysWithValues: elements)
  }

  /// 最小容量指定で初期化
  func testInitWithMinimumCapacity() {
    let dict = RedBlackTreeMap<String, Int>(minimumCapacity: 10)
