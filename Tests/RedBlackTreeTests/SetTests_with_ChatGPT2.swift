...
import XCTest
@testable import RedBlackTreeModule   // ←パッケージ名に合わせて変更！
...
final class RedBlackTreeSetAlgebraFullTests: XCTestCase {
    ...
    private func makeRandomPair(seed: inout SplitMix64) -> (
        ...
        return (RedBlackTreeSet(a), RedBlackTreeSet(b),
                Set(a), Set(b))
    }
    ...
    func testAllSetAlgebraOperationsEquivalence() {
        var rng = SplitMix64(seed: 0xFACEFEED)

        for _ in 0..<250 {    // 250 ランダムケース
            let (rbt1, rbt2, set1, set2) = makeRandomPair(seed: &rng)

            XCTAssertEqual(rbt1.union(rbt2).sorted(),
                           set1.union(set2).sorted())

            XCTAssertEqual(rbt1.intersection(rbt2).sorted(),
                           set1.intersection(set2).sorted())

            XCTAssertEqual(rbt1.symmetricDifference(rbt2).sorted(),
                           set1.symmetricDifference(set2).sorted())

            XCTAssertEqual(rbt1.subtracting(rbt2).sorted(),
    ...
    func testFormOperationsMutateReceiverOnly() {
        ...
        for _ in 0..<80 {
            ...
            var  sLU = sL ;  sLU.formUnion(sR)
            assertEquivalent(lhsU, RedBlackTreeSet(sLU))

            // formIntersection
            var lhsI = lhs; lhsI.formIntersection(rhs)
            var  sLI = sL ;  sLI.formIntersection(sR)
            assertEquivalent(lhsI, RedBlackTreeSet(sLI))

            // formSymmetricDifference
            var lhsS = lhs; lhsS.formSymmetricDifference(rhs)
            var  sLS = sL ;  sLS.formSymmetricDifference(sR)
            assertEquivalent(lhsS, RedBlackTreeSet(sLS))

            // subtract (mutating)
            var lhsSub = lhs; lhsSub.subtract(rhs)
            var  sLSub = sL ;  sLSub.subtract(sR)
    ...
    func testRelationOperationsMatchSwiftSet() {
        var rng = SplitMix64(seed: 0x0DDC0FFE)
