...
//
  ...
  final class SetBoundsExpressionTests: XCTestCase {
    ...
    func testAfter() throws {
      ...
      // 終端となる
      XCTAssertTrue(a._isEqual(start().after.after.after, end()))
      // 終端まではエラーが発生しない
      XCTAssertNil(a._error(start().after.after.after))

      XCTAssertFalse(a.isValid(start().after.after.after.after))
      XCTAssertEqual(a[start().after.after.after.after], nil)
      // 終端の次はnull相当
      XCTAssertFalse(a._isEqual(start().after.after.after.after, end()))
      // 終端の次は内部エラーとなる
      XCTAssertEqual(a._error(start().after.after.after.after), .upperOutOfBounds)
    }
