...
//
  ...
  final class SetBoundsExpressionTests: XCTestCase {
    ...
    func testBefore() throws {
      ...
      // エラーではない
      XCTAssertNil(a.error(end().before.before.before))
      XCTAssertFalse(a.isValid(end().before.before.before.before))

      // 内部エラー
      XCTAssertEqual(a.error(end().before.before.before.before), .lowerOutOfBounds)
    }

    func testAdvancePositive() throws {
      XCTAssertTrue(a.isValid(start()))
      XCTAssertTrue(a.isValid(start().advanced(by: 1)))
      XCTAssertTrue(a.isEqual(start().advanced(by: 1), start().after))
      XCTAssertTrue(a.isValid(start().advanced(by: 2)))
      XCTAssertTrue(a.isEqual(start().advanced(by: 2), start().after.after))

      XCTAssertFalse(a.isValid(start().advanced(by: 3)))
      XCTAssertTrue(a.isEqual(start().advanced(by: 3), start().after.after.after))
      XCTAssertTrue(a.isEqual(start().advanced(by: 3), end()))
      XCTAssertNil(a.error(start().advanced(by: 3)))

      XCTAssertFalse(a.isValid(start().advanced(by: 4)))
      XCTAssertTrue(a.isEqual(start().advanced(by: 4), start().after.after.after.after))
      XCTAssertEqual(a.error(start().advanced(by: 4)), .upperOutOfBounds)
      XCTAssertFalse(a.isEqual(start().advanced(by: 4), end()))
    }

    func testAdvanceNegative() throws {
      XCTAssertFalse(a.isValid(end()))
      XCTAssertTrue(a.isValid(end().advanced(by: -1)))
      XCTAssertTrue(a.isEqual(end().advanced(by: -1), end().before))
      XCTAssertTrue(a.isValid(end().advanced(by: -2)))
      XCTAssertTrue(a.isEqual(end().advanced(by: -2), end().before.before))
      XCTAssertTrue(a.isValid(end().advanced(by: -3)))
      XCTAssertTrue(a.isEqual(end().advanced(by: -3), end().before.before.before))
      XCTAssertFalse(a.isValid(end().advanced(by: -4)))
      XCTAssertTrue(a.isEqual(end().advanced(by: -4), end().before.before.before.before))
      XCTAssertFalse(a.isValid(end().advanced(by: -5)))
      XCTAssertTrue(a.isEqual(end().advanced(by: -5), end().before.before.before.before.before))
      XCTAssertEqual(a.error(end().advanced(by: -5)), .lowerOutOfBounds)
    }

    func testAdvance() throws {
      // 内部的にエラー
      XCTAssertEqual(a.error(start().advanced(by: -1)), .lowerOutOfBounds)
      // 内部的にエラー
      XCTAssertEqual(a.error(end().advanced(by: 1)), .upperOutOfBounds)
    }
    ...
    func testLimitedAdvance5() throws {
      XCTAssertEqual(
        a.error(start().advanced(by: 1).advanced(by: -5, limit: start().advanced(by: 2))),
        .lowerOutOfBounds)

      XCTAssertEqual(
        a.error(start().advanced(by: 2).advanced(by: -5, limit: start().advanced(by: 3))),
        .lowerOutOfBounds)

      XCTAssertEqual(
        a.error(start().advanced(by: 3).advanced(by: -5, limit: start().advanced(by: 4))),
        .lowerOutOfBounds)
    }
    ...
    func testLimitedAdvance6() throws {
      #if DEBUG
        XCTAssertEqual(
          a.error(start().advanced(by: 1).advanced(by: -5, limit: .debug(.null))),
          .lowerOutOfBounds)

        XCTAssertEqual(
          a.error(start().advanced(by: 2).advanced(by: -5, limit: .debug(.null))),
          .lowerOutOfBounds)

        XCTAssertEqual(
          a.error(start().advanced(by: 3).advanced(by: -5, limit: .debug(.null))),
          .lowerOutOfBounds)
      #endif

      #if DEBUG
        XCTAssertEqual(
          a.error(.debug(.null).advanced(by: -5, limit: .debug(.null))),
          .null)
      #endif
    }

    func testLowerBound() throws {
      XCTAssertTrue(a.isEqual(lowerBound(-2), start().advanced(by: 0)))
      XCTAssertTrue(a.isEqual(lowerBound(-1), start().advanced(by: 0)))
      XCTAssertTrue(a.isEqual(lowerBound(0), start().advanced(by: 0)))
      XCTAssertTrue(a.isEqual(lowerBound(1), start().advanced(by: 1)))
      XCTAssertTrue(a.isEqual(lowerBound(2), start().advanced(by: 2)))
      XCTAssertTrue(a.isEqual(lowerBound(3), start().advanced(by: 3)))
      XCTAssertTrue(a.isEqual(lowerBound(4), end()))
      XCTAssertTrue(a.isEqual(lowerBound(5), end()))
    }

    func testUpperBound() throws {
      XCTAssertTrue(a.isEqual(upperBound(-2), start().advanced(by: 0)))
      XCTAssertTrue(a.isEqual(upperBound(-1), start().advanced(by: 0)))
      XCTAssertTrue(a.isEqual(upperBound(0), start().advanced(by: 1)))
      XCTAssertTrue(a.isEqual(upperBound(1), start().advanced(by: 2)))
      XCTAssertTrue(a.isEqual(upperBound(2), start().advanced(by: 3)))
      XCTAssertTrue(a.isEqual(upperBound(3), end()))
      XCTAssertTrue(a.isEqual(upperBound(4), end()))
    }

    func testFind() throws {
      XCTAssertTrue(a.isEqual(find(-1), end()))
      XCTAssertTrue(a.isEqual(find(0), start().advanced(by: 0)))
      XCTAssertTrue(a.isEqual(find(1), start().advanced(by: 1)))
      XCTAssertTrue(a.isEqual(find(2), start().advanced(by: 2)))
      XCTAssertTrue(a.isEqual(find(3), start().advanced(by: 3)))
