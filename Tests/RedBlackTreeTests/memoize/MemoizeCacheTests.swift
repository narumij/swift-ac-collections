...
final class MemoizeCacheTests: XCTestCase {
  ...
  enum TestKey: _KeyCustomProtocol {
    ...
    func testMinimum() throws {
      ...
      XCTAssertEqual(cache._tree_.count, 0)
      XCTAssertEqual(cache._tree_.capacity, 10)
    }

    func testMaximum() throws {
      var cache = _MemoizeCacheBase<TestKey, Int>(minimumCapacity: 0)
      XCTAssertEqual(cache._tree_.count, 0)
      XCTAssertEqual(cache._tree_.capacity, 0)
      var finalCapacity: Int? = nil
      for i in 0..<200 {
        cache[i] = i
        if finalCapacity == nil, cache._tree_.capacity >= 100 {
          finalCapacity = cache._tree_.capacity
        }
        if let finalCapacity {
          // 最終的に定まったキャパシティが変化しない
          XCTAssertGreaterThanOrEqual(cache._tree_.capacity, finalCapacity, "\(i)")
        }
      }
    }
  #endif
