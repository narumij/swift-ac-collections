...
final class RedBlackTreeCopyOnWriteTests: XCTestCase {
  ...
  func testSet1() throws {
    ...
    XCTAssertEqual(set.copyCount, 0)
    set.insert(0)
    XCTAssertEqual(set.copyCount, 1) // 挿入に備えた分増える
    set.insert(0)
    XCTAssertEqual(set.copyCount, 2) // 挿入に備えた必要分が変化していない
    set.insert(0)
    XCTAssertEqual(set.copyCount, 2) // 挿入に備えた必要分が変化していない
    set.insert(1)
    XCTAssertEqual(set.copyCount, 2) // 挿入に備えた必要分を消費したところ
  }
  ...
  func testSet3() throws {
    ...
    XCTAssertEqual(tree.copyCount, 0) // これが0になる挙動にするか、1になる挙動にするか、悩み
  }
  ...
  func testSet4() throws {
    ...
    XCTAssertEqual(tree.count, 0)
    XCTAssertEqual(tree.copyCount, 1)
  }
  ...
  func testSet5() throws {
    ...
    XCTAssertEqual(tree.count, 0)
    XCTAssertEqual(tree.copyCount, 0)
  }
  ...
  func testSet3000() throws {
    ...
    XCTAssertTrue(xy[1]!.checkUnique())
    XCTAssertEqual(xy[1]!.count, 0)
    XCTAssertEqual(xy[1]!.copyCount, 0)
